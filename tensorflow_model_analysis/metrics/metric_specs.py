...
def metric_keys_to_skip_for_confidence_intervals(
  ...
  skipped_keys = [
      key for key, _, instance in keys_and_metrics_from_specs(
          ...
          for sub_key in child_sub_keys:
            ...
            for model_name in spec.model_names or ['']:
              unique_args = UniqueArgs(
                  model_name,
                  sub_key,
                  None if is_macro else aggregation_type,
                  () if is_macro else class_weights,
              )
      ...
      for aggregation_type, sub_keys in sub_keys_by_aggregation_type.items():
        ...
        if is_macro := (aggregation_type
          ...
          if not updated_sub_keys:
            continue
          aggregation_type = None if is_macro else aggregation_type
          class_weights = None
...
def _private_tf_metric(
  ...
  if not cfg['config']['name'].startswith('_'):
    cfg['config']['name'] = f'_{cfg["config"]["name"]}'
